...
# db: AsyncSession 這個參數必須使用通道規格屬性
async def get_user_by_email(db: AsyncSession, email: str) -> User | None:
    ...
    # 記錄查詢結果
    print(f"[會員服務] 查詢結果：{'找到' if user else '找不到'}")  
    return user
#endregion
...
async def create_user(
    ...
    # 將物件加入當前 Session（標記為待寫入）
    # db.add跟db.commit 是配套使用 
    # add作用像是放入購物車但還沒有真的付錢
    # commit作用是確認過後真的付錢
    db.add(user)       
    # 執行 INSERT，將資料正式寫入資料庫 
    await db.commit()   
     # 從資料庫重新載入物件，取得 id 等自動產生的欄位
...
async def check_email_exists(db: AsyncSession, email: str) -> bool:
    ...
    # 記錄確認開始
    print(f"[會員服務] 確認 email 是否存在：{email}")  

    # 直接呼叫 get_user_by_email查詢email
    user = await get_user_by_email(db, email)

    # True 表示已存在，False 表示可以註冊
    result = user is not None  

    # 記錄確認結果
    print(f"[會員服務] 確認結果：{'已存在' if result else '可以註冊'}")  
